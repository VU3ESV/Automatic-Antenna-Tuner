...
// Automatic-Antenna-Tuner patch: RAM bounce buffer for page-sized writes in
// flash_move() (see the comment there). Lives in DTCM like all .bss.
#define FLASH_MOVE_PAGE 256
static uint8_t flash_move_page[FLASH_MOVE_PAGE] __attribute__ ((aligned (4)));
#endif
...
RAMFUNC void flash_move( uint32_t dst, uint32_t src, uint32_t size )
{
  ...
  #if defined(__IMXRT1062__)
  // Automatic-Antenna-Tuner patch: on the i.MX RT the core's flash_wait()
  // re-enables interrupts at the end of every eepromemu_flash_* call, so
  // upstream only had them masked inside each erase / program. Everything
  // this function touches lives in ITCM / DTCM, so mask them here and
  // re-assert after every core call: the sector holding the FLASHMEM /
  // PROGMEM data (USB descriptors) is erased early in the copy and no ISR
  // may reach it. REBOOT needs no interrupts.
  __disable_irq();
  #endif
  ...
  while (offset < size && error == 0) {
    ...
    // FSEC location was written by the code above, so use aFSEC=1, oFSEC=0
      ...
      // Automatic-Antenna-Tuner patch: program one 256-byte flash page per
      // operation instead of upstream's 4 bytes. The W25Q64 page-program
      // time (~0.4 ms) does not depend on the byte count, so the ~66 000
      // word programs upstream needed for a 270 KB image (~25 s) become
      // ~1 060 page programs (~0.5 s). The copy is now dominated by the
      // sector erases — 67 for the program area plus 67 for the buffer
      // below, 45 ms typical / 400 ms max each on the W25Q64JV — so the
      // whole cannot-lose-power window is several seconds, not sub-second.
      // Pages never straddle a sector (4096 % 256 == 0), so the
      // erase-before-write check above still runs at every sector start.
      // Word copy (no libc call) keeps this RAMFUNC self-contained; a
      // trailing partial word reads erased 0xFF bytes from the buffer,
      // which program as no-ops.
      uint32_t n = FLASH_MOVE_PAGE;
      if (size - offset < n) n = size - offset;
      for (uint32_t i = 0; i < n; i += 4)
        *(uint32_t *)(flash_move_page + i) = *(uint32_t *)(src + offset + i);
      eepromemu_flash_write( (void*)addr, flash_move_page, n );
      __disable_irq();
      offset += n;
    #elif (FLASH_WRITE_SIZE==4)
  ...
  if (IN_FLASH(src)) {
    #if defined(__IMXRT1062__)
    // Automatic-Antenna-Tuner patch: the page copy above leaves offset ==
    // size exactly; the loop below steps by 4 and tests sector alignment,
    // so an image whose size is not a multiple of 4 would never match.
    offset = (offset + 3) & ~3u;
    #endif
