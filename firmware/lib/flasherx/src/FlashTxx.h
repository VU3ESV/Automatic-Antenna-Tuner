...
#include <stdint.h>     // uint32_t, etc.
  ...
  // Automatic-Antenna-Tuner patch: project-specific id (upstream "fw_teensy41")
  // so an unrelated Teensy 4.1 sketch that also links FlasherX is refused.
  #define FLASH_ID		"fw_aat_teensy41"	// target ID (in code)
  #define FLASH_SIZE		(0x800000)		// 8MB
  #define FLASH_SECTOR_SIZE	(0x1000)		// 4KB sector size
  #define FLASH_WRITE_SIZE	(4)			// 4-byte/32-bit writes    
  // Automatic-Antenna-Tuner patch: upstream reserves 4 sectors (16 KB).
  // Teensyduino's EEPROM emulation (cores/teensy4/eeprom.c) owns the top
  // 64 sectors of the 8 MB flash (0x607C0000..0x607FFFFF, 256 KB) and this
  // project keeps its settings + position anchors there, so the staging
  // buffer — and the post-update erase in flash_move() — must stop below it.
  #define FLASH_RESERVE		(64*FLASH_SECTOR_SIZE)	// reserve top of flash (EEPROM emulation)
  #define FLASH_BASE_ADDR	(0x60000000)		// code starts here
