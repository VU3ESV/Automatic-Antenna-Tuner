...
|------------------|-------------------------------------------|--------------------------------------------------------|
| `hal::motor`     | `motor_sim.cpp` — virtual stepper, advances kStepsPerTick toward target each `motor::tick()`. | iHSS60 integrated closed-loop drive (STEP/DIR/EN into its opto inputs) behind `TARGET_TEENSY41`; step pulses generated by **FlexPWM** with reload-IRQ step counting (per-axis). The bench reference impl in [`firmware/t41-stepper-test/src/flexpwm_stepper.h`](../firmware/t41-stepper-test/src/flexpwm_stepper.h) is the prototype — copy that pattern. Timing per the CLAUDE.md hardware contract: DIR ≥ 6 µs setup / ≥ 5 µs hold, ≥ 2.5 µs per pulse level, `dsb` after the flag clear in the reload ISR (bench findings in PROPOSAL.md). |
